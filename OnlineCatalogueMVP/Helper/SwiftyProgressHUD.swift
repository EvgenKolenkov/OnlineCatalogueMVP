...
extension UIViewController {
    public var progressHUD: SwiftyProgressHUD? {
        return view.progressHUD
    }
    
    public var isContainsHUD: Bool {
        return view.isContainsHUD
    }
...
public class SwiftyProgressHUD: UIView {
    ...
    public class func showInView(_ view : UIView, animated : Bool) {
        let hud = SwiftyProgressHUD(frame: view.frame)
        view.addSubview(hud)
    ...
    func updateIndicators() {
        activityIndicator.center = bezelView.center
    ...
    override public func layoutSubviews() {
        ...
        let offset = superview.frame.origin.y
        frame = superview.bounds
        backgroundView.frame = frame
        
        var bezeFrame = CGRect(x: frame.size.width/2-40,
                               y: (frame.size.height - offset)/2-40,
                               width: 80, height: 80)
